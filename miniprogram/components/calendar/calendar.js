...
Component({
  ...
  methods: {
    ...
    getWeek(dateData, isNeedChangeCurrent=false) {
      ...
      if(!isNeedChangeCurrent){
        this.triggerEvent('getdate', { year, month, date })
      }else {
        let year = (new Date()).getFullYear(); //年
        let month = (new Date()).getMonth() + 1  < 10 ? "0" +((new Date()).getMonth() + 1) : ((new Date()).getMonth() + 1) ;  //月
        let date = (new Date()).getDate() < 10 ? "0"+(new Date()).getDate(): (new Date()).getDate();//日
        this.triggerEvent('getdate', { year, month, date })
      }
    },
    /**
